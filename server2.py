...
def write_to_file(data):
    with open ('server2.data', 'w') as f:
        for tempdata in data:
            f.write("%s %s %s \n" % (tempdata.get_tid(),
                        tempdata.get_act(), tempdata.get_bal()))

def logginto_file(data):
    with open ('server2.log', 'a') as f:
	transid = data.get_qtid()
	temptransid = transid + 2
	data.set_qtid(temptransid)
...
def send_diff_to_server(data, index) :
    ...
    diffdata.sentfrom = "server2"
    if index == 0:
        diffdata.sentto = "server1"
    if index == 2 :
...
def initiate_resync():
    ...
    resyncdata.data = server2_data
    resyncdata.length = len(resyncdata.data)
    resyncdata.sentfrom = "server2"
...
def keep_receiving():
    while True:
        ...
        if resultpickled.packettype == RESYNCRONIZE:
            ...
            if G.get_SDSEND() and G.get_htbt1() > 10:
                send_resync_done()
                G.set_htbt1(0)
                G.set_SDSEND(0)    
		    ...
		    temptransid = serverdata.get_tid() + 2
		    serverdata.set_tid(temptransid)			# transid
                    resultpickled.data.set_qtid(serverdata.get_tid())
